...
export default function CoverLetter() {
  return (
    <section className={`${card.cardContainer} ${card.card}`}>
      <h2 className={card.title}>자기소개서</h2>
      <h3 className={card.subTitle}>실행력</h3>
      <p className={card.content}>
        저는 빠른 실행력으로{" "}
        <span className={card.bold}>소비자가 원하는 것을 만드는 개발자</span>가
        되기 위해 노력하고 있습니다. 시장에 맞춰 빠르게 필요한 기술을 배워
        기능을 만들어내는 것이 중요하다고 생각합니다. 이를 위해 새로운 도구나
        방법을 학습하는 것을 꺼리지 않으며, 그것이 더 나은 결과를 가져올 수
        있다면 적극적으로 받아들입니다.
      </p>
